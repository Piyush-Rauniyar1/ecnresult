#!/usr/bin/env node
/**
 * Scraper for remaining constituencies with better logging
 */
...
async function fetchAndParseConstituency(constId: number, url: string): Promise<{ success: boolean; count: number; withVotes: number; error?: string }> {
    try {
        ...
        console.log(`  Updated ${createdCount} results in database`);
...
async function main() {
    ...
    for (const item of URLS_TO_SCRAPE) {
        ...
        console.log(`⏳ Scraping ${item.name} (ID: ${constId})...`);
        
        const result = await fetchAndParseConstituency(constId, item.url);
        
        if (result.success) {
            console.log(`✅ ${item.name} Updated (${result.count} candidates, ${result.withVotes} with votes)\n`);
            successCount++;
        } else {
