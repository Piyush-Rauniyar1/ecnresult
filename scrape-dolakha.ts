...
async function getOrCreateParty(partyName: string): Promise<number | null> {
    ...
    if (error) {
        console.error(`Error creating party ${partyName}:`, error.message);
        return null;
    }
...
async function getOrCreateCandidate(constId: number, partyId: number | null, cand: any): Promise<number | null> {
    ...
    if (error) {
        console.error(`Error creating candidate ${cand.name}:`, error.message);
        return null;
    }
...
async function main() {
    console.log('🔄 Scraping Dolakha-1...\n');
    ...
    const constId = constMap.get('Dolakha-1');
    
    if (!constId) {
        console.log(`❌ Dolakha-1 - Constituency not found`);
        process.exit(1);
    }

    const result = await fetchAndParseConstituency(
        constId, 
        'https://election.ekantipur.com/pradesh-3/district-dolkha/constituency-1?lng=eng'
    );
    
    if (result.success) {
        console.log(`✅ Dolakha-1 Updated (${result.count} candidates, ${result.withVotes} with votes)`);
    } else {
        console.log(`❌ Dolakha-1 Failed: ${result.error}`);
        process.exit(1);
    }
}
